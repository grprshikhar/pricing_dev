...
#    df_init.loc[ (df_init["rrp"]>=mp_critical), "mp_critical"] = 1
    ...
    phcu.d_csp.bulky = phcu.d_csp.bulky.astype(int)
    df_init = df_init.merge(phcu.d_csp.drop_duplicates(), on=['category','subcategory','store code','bulky'])
...
def check_rrp_perc(df_td,plan_limit_dict):
    ...
    confirm_perc_check='xsss'
    while confirm_perc_check=='xsss':
        print("\n")
        confirm_perc_check1 = input("Check Percentage Guideline (yes/no/exit) (exit program): ")
        confirm_perc_check1 = confirm_perc_check1.lower()
        if confirm_perc_check1 not in ['yes','no','exit']:
            print(colored("Incorrect response!",'red'),"\n\nPlease use only (yes/no/exit)\n")
        elif confirm_perc_check1=='exit' :
            print("\n\nYou selected 'exit' or you are not authorized here. Exiting wizard now...\n\nBye!\n\n")
            sys.exit()
        elif confirm_perc_check1=='no' :
            confirm_perc_check = confirm_perc_check1
        else:
