...
def check_EU_rules(df, df_dsd, df_30day, df_median_high_price_30day):
    ...
    for market, sku in market_sku:
        ...
        try:
            discount_allowed = (True if df_dsd.loc[(df_dsd["product_sku"] == sku), "new_discount_allowed"].values[0] == 1 else False)
        # If the SKU is not present, it means no discount ever made, so its fine to apply
        except:
            discount_allowed = True
        # Lowest pricing data
        lowest_prices = df_30day.loc[(df_30day["store_parent"] == market) & (df_30day["product_sku"] == sku)]
        ...
        for rp in rp_plans:
            ...
            # Retrieve values
            active_low_price       = df_copy.loc[(df_copy["store code"] == market) & (df_copy["sku"] == sku), col_active_low_price].values[0]
            active_high_price      = df_copy.loc[(df_copy["store code"] == market) & (df_copy["sku"] == sku), col_active_high_price].values[0]
            old_low_price          = df_copy.loc[(df_copy["store code"] == market) & (df_copy["sku"] == sku), col_old_low_price].values[0]
            old_high_price         = df_copy.loc[(df_copy["store code"] == market) & (df_copy["sku"] == sku), col_old_high_price].values[0]

            # Skip if nan because this means plan is not being used or not applying a discount
            ...
            # NEW CHECK: Check if the current plan high price is greater than median high price
            col_median_high_price_30day = f"m{rp}_median_high_price"
            try:
                median_high_price_30day = float(df_median_high_price_30day.loc[(df_median_high_price_30day["product_sku"] == sku), col_median_high_price_30day].values[0])
                if active_high_price > median_high_price_30day:
                    any_warnings.append(f"{market} {sku} {rp:2}M rental plan : High price violates EU law : [{active_high_price} vs median {median_high_price_30day}]")
            except:
                any_warnings.append(f"{market} {sku} {rp:2}M rental plan : Median high price for the past 30 days is missing. Please report if this is not a new SKU!")
            ...
            # If yes - Check that new low price is lower than old low price
            if prev_discount:
                is_new_price_lower = (active_low_price < old_low_price)
                # If yes - This is fine to proceed - Successive Discount (3 month to worry later)
