def market_price_scraper_v02_EU():
  # Imports
  ...
  print_exclaim("Updating Competition Pricing for EU")
  ...
  inhouse_market_prices = gsheet.get_dataframe("13fvWqCqV1cgt6vrpg5INJzX0haBx2znwwt5VM_30j0A","A:B","Market Price")
...
def market_price_scraper_BO():
  ...
  last_run = pd.read_sql_query('SELECT *, JULIANDAY(CURRENT_DATE) - JULIANDAY(crawl_date) as datediff FROM data_output where datediff between 0 and 30 ',conn)
  last_run_date = last_run["crawl_date"].loc[0]
